...
for i=selectedPointsR%2:4:80%wybrane indeksy punktów na ramce w pobli¿u których szukamy brzegu
    ...
    if FitFresnel
        ...
        x0=[1 25 8 0];
        [x,resnorm,residual,exitflag,output] = lsqcurvefit(fun,x0,xdata,ydata',[-1 1 1e-6 -1],[1 j 16 1],optimoptions('lsqcurvefit','Diagnostics','on','Display','final-detailed','ScaleProblem','jacobian','TolFun',1e-16));
        meanx=mean(ydata);
